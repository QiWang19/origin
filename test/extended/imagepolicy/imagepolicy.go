...
var _ = g.Describe("[sig-imagepolicy][OCPFeatureGate:SigstoreImageVerification][Serial]", g.Ordered, func() {
	...
	g.AfterAll(func() {
		err := deleteClusterImagePolicy(oc, publiKeyRekorClusterImagePolicyFixture)
		o.Expect(err).NotTo(o.HaveOccurred())
		// err := deleteClusterImagePolicy(oc, publiKeyRekorClusterImagePolicyFixture)
		// o.Expect(err).NotTo(o.HaveOccurred())

		err = deleteImagePolicy(oc, publiKeyRekorImagePolicyFixture, clif.Namespace.Name)
		o.Expect(err).NotTo(o.HaveOccurred())

		err = deleteImagePolicy(oc, publiKeyRekorImagePolicyFixture, imgpolicyClif.Namespace.Name)
		o.Expect(err).NotTo(o.HaveOccurred())

		err = deleteImagePolicy(oc, invalidPublicKeyImagePolicyFixture, clif.Namespace.Name)
		o.Expect(err).NotTo(o.HaveOccurred())

		err = deleteImagePolicy(oc, invalidPublicKeyImagePolicyFixture, imgpolicyClif.Namespace.Name)
		o.Expect(err).NotTo(o.HaveOccurred())
	})
})
